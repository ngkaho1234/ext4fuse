 ...
 */
void rb_augment_insert(struct rb_node *node, rb_augment_f func,
			 void *data)
{
	if (node->rb_left)
		node = node->rb_left;
 ...
 */
void rb_augment_erase_end(struct rb_node *node, rb_augment_f func,
			    void *data)
{
	if (node)
		rb_augment_path(node, func, data);
...
void rb_erase(struct rb_node *node, struct rb_root *root)
{
	...
	if (node == root->rb_rightmost)
		root->rb_rightmost = rb_prev(node);

	__rb_erase(node, root);
}
