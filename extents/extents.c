...
static inline int ext4_ext_space_block(struct inode *inode, int check)
{
	...
	size = (super_block_size() - sizeof(struct ext4_extent_header))
...
void print_path(struct ext4_ext_path *path)
{
	...
	ext_debug("====================\n");
	while (i >= 0) {
		ext_debug("depth %d, p_block: %llu, p_ext offset: %d, p_idx offset: %d\n", i,
			path->p_block,
